...
public class TracerSystem : MonoBehaviour
{
    ...
    void Start() => tracers = new ObjectPool<GameObject>(
        CreatePooledItem,
        OnTakeFromPool,
        OnReturnedToPool,
        OnDestroyPoolObject,
        true, poolSize, maxPoolSize
    );
    ...
    void OnReturnedToPool(GameObject tracer) => tracer.gameObject.SetActive(false);
    
    void OnTakeFromPool(GameObject tracer) => tracer.gameObject.SetActive(true);

    void OnDestroyPoolObject(GameObject tracer) => Destroy(tracer);
